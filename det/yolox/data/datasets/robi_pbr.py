...
DATASETS_ROOT = osp.normpath(osp.join(PROJ_ROOT, "../data/ROBI/bop"))


class ROBI_PBR_Dataset:
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg.get(
            "dataset_root", osp.join(DATASETS_ROOT, "robi_20000/train")
        )
        assert osp.exists(self.dataset_root), self.dataset_root
        self.xyz_root = data_cfg.get(
            "xyz_root", osp.join(self.dataset_root, "xyz_crop")
        )
        ...
        self.cat_ids = [
            cat_id
            for cat_id, obj_name in ref.robi.id2obj.items()
            if obj_name in self.objs
        ]
        # map selected objs to [0, num_objs)
        self.cat2label = {v: i for i, v in enumerate(self.cat_ids)}  # id_map
        self.label2cat = {label: cat for cat, label in self.cat2label.items()}
        self.obj2label = OrderedDict(
            (obj, obj_id) for obj_id, obj in enumerate(self.objs)
        )

        scene_dirs = [
            d
            for d in glob.glob(os.path.join(self.dataset_root, "[0-9]*"))
            if os.path.isdir(d)
        ]
        self.scenes = [os.path.basename(scene_dir) for scene_dir in scene_dirs]
    ...
    def __call__(self):
        ...
        for scene in tqdm(self.scenes):
            ...
            for str_im_id in tqdm(gt_dict, postfix=f"{scene_id}"):
                ...
                K = np.array(cam_dict[str_im_id]["cam_K"], dtype=np.float32).reshape(
                    3, 3
                )
                ...
                for anno_i, anno in enumerate(gt_dict[str_im_id]):
                    ...
                    mask_file = osp.join(
                        scene_root, "mask/{:06d}_{:06d}.png".format(int_im_id, anno_i)
                    )
                    ...
                    if area < 30:  # filter out too small or nearly invisible instances
                        ...
                        continue
                    visib_fract = gt_info_dict[str_im_id][anno_i].get(
                        "visib_fract", 1.0
                    )
        ...
        logger.info(
            "loaded {} dataset dicts, using {}s".format(
    ...
    def models(self):
        ...
        cache_path = osp.join(
            self.models_root, "models_{}.pkl".format("_".join(self.objs))
        )
        if osp.exists(cache_path) and self.use_cache:
...
def get_robi_metadata(obj_names, ref_key):
...
robi_model_root = f"{DATASETS_ROOT}/robi/models/"
################################################################################


SPLITS_ROBI_PBR = dict(
    robi_pbr_train=dict(
        name="robi_pbr_train",
        objs=ref.robi.objects,  # selected objects
        dataset_root=osp.join(DATASETS_ROOT, "robi_20000/train"),
        models_root=osp.join(DATASETS_ROOT, "robi_20000/models"),
        xyz_root=osp.join(DATASETS_ROOT, "robi_20000/train/xyz_crop"),
...
# single obj splits
for obj in ref.robi.objects:
    for split in ["train"]:
        name = "robi_pbr_{}_{}".format(obj, split)
        if split in ["train"]:
            filter_invalid = True
        elif split in ["test"]:
            filter_invalid = False
        ...
        if name not in SPLITS_ROBI_PBR:
            SPLITS_ROBI_PBR[name] = dict(
                name=name,
                objs=[obj],  # only this obj
                dataset_root=osp.join(DATASETS_ROOT, "robi_20000/train"),
                models_root=osp.join(DATASETS_ROOT, "robi_20000/models_cad"),
                xyz_root=osp.join(DATASETS_ROOT, "robi_20000/train/xyz_crop"),
                scale_to_meter=0.001,
                with_masks=True,  # (load masks but may not use it)
                with_depth=True,  # (load depth path here, but may not use it)
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_ROBI_PBR:
        used_cfg = SPLITS_ROBI_PBR[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, ROBI_PBR_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="robi",  # NOTE: for pvnet to determine module
        ref_key=used_cfg["ref_key"],
        objs=used_cfg["objs"],
        eval_error_types=["ad", "rete", "proj"],
        evaluator_type="bop",
        **get_robi_metadata(obj_names=used_cfg["objs"], ref_key=used_cfg["ref_key"]),
    )


def get_available_datasets():
    return list(SPLITS_ROBI_PBR.keys())
...
def test_vis():
    ...
    logger.info(
        "Done loading {} samples with {:.3f}s.".format(
