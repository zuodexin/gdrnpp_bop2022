...
def load_ply_bop(model_path, vertex_scale=1.0):
    model = load_ply_bop_(model_path)
    model["pts"] = model["pts"] * vertex_scale
    return model


def load_ply_bop_(path):
    ...
    model["pts"] = np.zeros((n_pts, 3), np.floating)
    if n_faces > 0:
        model["faces"] = np.zeros((n_faces, face_n_corners), np.floating)
    ...
    if {"nx", "ny", "nz"}.issubset(set(pt_props_names)):
        is_normal = True
        model["normals"] = np.zeros((n_pts, 3), np.floating)

    is_color = False
    if {"red", "green", "blue"}.issubset(set(pt_props_names)):
        is_color = True
        model["colors"] = np.zeros((n_pts, 3), np.floating)
    ...
    if {"texcoord"}.issubset(set(face_props_names)):
        ...
        model["texture_uv_face"] = np.zeros((n_faces, 6), np.floating)
