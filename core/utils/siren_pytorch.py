...
class SirenNet(nn.Module):
    ...
    def forward(self, x, muls=None, adds=None):
        muls = cast_tuple(muls, self.num_layers)
        adds = cast_tuple(adds, self.num_layers)

        for layer, mul, add in zip(self.layers, muls, adds):
            x = layer(x)
