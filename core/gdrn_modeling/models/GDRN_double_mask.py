...
from lib.torch_utils.layers.reverse_pos import PosEncoding
...
class GDRN_DoubleMask(nn.Module):
    ...
    def forward(
        ...
        xyz_pe = None
        if self.xyz_type == "pe":
            # decode xyz from pos encoding
            vis_mask, full_mask, region, xyz_pe = self.geo_head_net(conv_feat)
            b, c, d, p, h, w = xyz_pe.shape
            xyz_pe = xyz_pe[torch.arange(bs).to(device), roi_classes]
            xyz_pe_flat = rearrange(xyz_pe, "b d p h w -> b h w d p").reshape(-1, d * p)
            xyz = self.decode_pos(xyz_pe_flat)
            xyz = xyz.reshape(b, h, w, 3).permute(0, 3, 1, 2)
            coor_x = xyz[:, 0, :, :].view(
                bs, self.xyz_out_dim // 3, out_res, out_res
            )
            coor_y = xyz[:, 1, :, :].view(
                bs, self.xyz_out_dim // 3, out_res, out_res
            )
        ...
        elif self.xyz_type == "coordinate":
            vis_mask, full_mask, coor_x, coor_y, coor_z, region = self.geo_head_net(
                conv_feat
            )
        else:
            raise ValueError(f"Unknown xyz type: {self.cfg.XYZ_TYPE}")

        if g_head_cfg.XYZ_CLASS_AWARE and self.xyz_type == "coordinate":
    ...
    def gdrn_loss(
        ...
        if loss_cfg.XYZ_LOSS_SYM:
            ...
            for _i in range(bs):
                dist = (
                    torch.norm(pred_xyz[_i].unsqueeze(0) - gt_xyz_sym[_i], dim=1)
                    .flatten(1)
                    .mean(1)
                )
        ...
        # xyz_pe loss ----------------------------------
        if not g_head_cfg.FREEZE and self.xyz_type == "pe":
            # gt_xyz to encoding
            bs, c, h, w = gt_xyz.shape
            gt_xyz_pe = gt_xyz.permute(0, 2, 3, 1).reshape(bs * h * w, c)
            gt_xyz_pe = self.encode_pos(gt_xyz_pe)
            gt_xyz_pe = gt_xyz_pe.reshape(bs, h, w, -1).permute(0, 3, 1, 2)

            gt_mask_xyz = gt_masks[loss_cfg.XYZ_LOSS_MASK_GT]
            loss_func = nn.L1Loss(reduction="none")
            
            xyz_pe_loss = loss_func(
                xyz_pe.flatten(1,2) * gt_mask_xyz[:, None], gt_xyz_pe * gt_mask_xyz[:, None]
            ).mean(1).sum() / gt_mask_xyz.sum().float().clamp(min=1.0)
            loss_dict["loss_xyz_pe"] = xyz_pe_loss

        if not g_head_cfg.FREEZE and self.xyz_type == "coordinate":
        ...
        if loss_cfg.REGION_LOSS_SYM:
            ...
            for _i in range(bs):
                ...
                dist = nn.CrossEntropyLoss(reduction="none")(
                    out_region[_i].unsqueeze(0).expand(n_sym, -1, -1, -1),
    ...
    @torch.no_grad()
    def decode_pos(self, pe):
        bs, pd = pe.shape
        pe = pe.reshape(bs * 3, -1)
        x = self.pos_xyz.reverse(pe).view(bs, -1)
        return x

    @torch.no_grad()
    def encode_pos(self, x):
        bs, d = x.shape
        pe = self.pos_xyz(x.reshape(-1)).view(bs, -1)
        return pe
