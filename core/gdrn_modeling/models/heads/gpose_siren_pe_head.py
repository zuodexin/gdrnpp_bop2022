...
class GPoseDoubleMaskSirenPEHead(nn.Module):
    def __init__(
        self,
        in_dim,
        ...
        self.siren = SirenNet(
            ...
            * self.pos_dim
            * self.xyz_num_classes,  # output dimension, ex. rgb value
    ...
    def forward(self, x):
        ...
        # predict xyz
        xyz_dim = self.xyz_out_dim * self.pos_dim * self.xyz_num_classes
        b, c, h, w = x.shape
        xyz_pe = (
            self.siren(x.permute(0, 2, 3, 1).reshape(b * h * w, c))
            .reshape(b, h, w, xyz_dim)
            .permute(0, 3, 1, 2)
        )
        xyz_pe = xyz_pe.reshape(
            b, self.xyz_num_classes, self.xyz_out_dim, self.pos_dim, h, w
        )
        return vis_mask, full_mask, region, xyz_pe
