...
class ROBI_TEST_Dataset(object):
    """robi test."""
    ...
    def __init__(self, data_cfg):
        ...
        self.obj2label = OrderedDict(
            (obj, obj_id) for obj, obj_id in enumerate(self.objs)
        )
        ##########################################################
...
SPLITS_ROBI = dict(
    robi_test=dict(
        ...
        models_root=ref.robi.model_dir,
        objs=ref.robi.objects,  # selected objects
        ann_file=osp.join(DATASETS_ROOT, "robi_10000/test_targets_bop19.json"),
...
for obj in ref.robi.objects:
    name = "robi_{}_test".format(obj)
    select_objs = [obj]
    if name not in SPLITS_ROBI:
        SPLITS_ROBI[name] = dict(
            name=name,
            dataset_root=osp.join(DATASETS_ROOT, "robi_10000/test_primesense"),
            models_root=ref.robi.model_dir,
            objs=[obj],
            ann_file=osp.join(DATASETS_ROOT, "robi_10000/test_targets_bop19.json"),
            scale_to_meter=0.001,
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_ROBI:
        used_cfg = SPLITS_ROBI[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, ROBI_TEST_Dataset(used_cfg))
